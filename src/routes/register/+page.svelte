<script lang="js">
    ...
    async function submit(e) {
        if (username === "" || username === null){
            e.preventDefault()
            errorCode = [true,"Please input an username!"]
        }
        if (email === "" || email === null) {
            e.preventDefault();
        ...
        if (password !== confirmPassword) {
            e.preventDefault();
            errorCode = [true, "Password must be the same!"];
            return;
        }
        const res = await fetch("http://localhost/spacefood/register.php", {
            method: "POST",
            headers: {
...
<svelte:head>
  <title>Register to SPACEFOOD!</title>
</svelte:head>
        ...
        <div class="w-full max-w-lg md:shadow-lg md:p-8 md:rounded-xl">
            <div class="text-center">
                <h1 class="text-2xl font-bold mb-3">
                    Register to<br /><span class="logo text-3xl text-orange-400"
                        >SPACEFOOD</span
                    >
                </h1>

                <p class="text-sm font-light opacity-60 mt-2">
                    Keep updated to the newest event
                </p>
            </div>
