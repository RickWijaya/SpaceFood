<script lang="ts">
  const props = $props();
  const editable = props.data.editable.recipe;
  console.log(editable)
  // Initialize form with existing recipe data
  let name = $state(editable.title);
  let desc = $state(editable.content);
  let duration = $state(editable.duration);
  let image ="";
  let uploadURL = $state(editable.thumbnail_url);

  // Parse steps into simple array
  ...
  // If no steps, start with one empty field
  ...
  editable.ingredients.map(ing => {
    // Split the quantity into amount and unit
    const parts = ing.quantity.split(/(\d+|\d+\.\d+|\d+\/\d+)/).filter(Boolean);
    const amount = parts.length > 0 ? parts[0].trim() : "";
    const unit = parts.length > 1 ? parts[1].trim() : "pc";
    
    return {
      name: ing.name,
      amount: amount,
      unit: unit || "pc" // Default to "pc" if unit is empty
    };
  })
);
if (ingredient.length === 0) {
  ...
  const updateEvent = async (e: Event) => {
    ...
    formData.append("recipe_id", editable.recipe_id);

    try {
        const res = await fetch("http://localhost/spacefood/updaterecipe.php", {
            method: "POST",
            body: formData,
            credentials: "include"
        });
